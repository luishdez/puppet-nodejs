...
#
# [*service_autorestart*]
#   Automatically restarts the nodejs service when there is a change in
...
#   Can be defined also by the (top scope) variable $nodejs_disable
#
# [*disableboot*]
#   Set to 'true' to disable service(s) at boot, without checks if it's running
#   Use this when the service is managed by a tool like a cluster software
#   Can be defined also by the (top scope) variable $nodejs_disableboot
#
# [*monitor*]
#   Set to 'true' to enable monitoring of the services provided by the module
#   Can be defined also by the (top scope) variables $nodejs_monitor
#   and $monitor
#
# [*monitor_tool*]
#   Define which monitor tools (ad defined in Example42 monitor module)
#   you want to use for nodejs checks
#   Can be defined also by the (top scope) variables $nodejs_monitor_tool
#   and $monitor_tool
#
# [*monitor_target*]
#   The Ip address or hostname to use as a target for monitoring tools.
#   Default is the fact $ipaddress
#   Can be defined also by the (top scope) variables $nodejs_monitor_target
#   and $monitor_target
#
# [*puppi*]
#   Set to 'true' to enable creation of module data files that are used by puppi
#   Can be defined also by the (top scope) variables $nodejs_puppi and $puppi
#
# [*puppi_helper*]
...
#   Can be defined also by the (top scope) variables $nodejs_puppi_helper
#   and $puppi_helper
#
# [*firewall*]
#   Set to 'true' to enable firewalling of the services provided by the module
#   Can be defined also by the (top scope) variables $nodejs_firewall
#   and $firewall
#
# [*firewall_tool*]
#   Define which firewall tool(s) (ad defined in Example42 firewall module)
#   you want to use to open firewall for nodejs port(s)
#   Can be defined also by the (top scope) variables $nodejs_firewall_tool
#   and $firewall_tool
#
# [*firewall_src*]
#   Define which source ip/net allow for firewalling nodejs. Default: 0.0.0.0/0
#   Can be defined also by the (top scope) variables $nodejs_firewall_src
#   and $firewall_src
#
# [*firewall_dst*]
#   Define which destination ip to use for firewalling. Default: $ipaddress
#   Can be defined also by the (top scope) variables $nodejs_firewall_dst
#   and $firewall_dst
#
# [*debug*]
#   Set to 'true' to enable modules debugging
#   Can be defined also by the (top scope) variables $nodejs_debug and $debug
#
# [*audit_only*]
...
#
# [*service*]
#   The name of nodejs service
#
# [*service_status*]
#   If the nodejs service init script supports status argument
#
# [*process*]
#   The name of nodejs process
#
# [*process_args*]
#   The name of nodejs arguments. Used by puppi and monitor.
#   Used only in case the nodejs process name is generic (java, ruby...)
#
# [*process_user*]
#   The name of the user nodejs runs with. Used by puppi and monitor.
#
# [*config_dir*]
...
#   Can be defined also by the (top scope) variable $nodejs_port
#
# [*protocol*]
#   The protocol used by the the service.
#   This is used by monitor, firewall and puppi (optional) components
#   Can be defined also by the (top scope) variable $nodejs_protocol
#
#
# == Examples
#
...
class nodejs (
  ...
  file { 'nodejs.conf':
    ...
    notify  => $nodejs::manage_service_autorestart,
  ...
  if $nodejs::bool_puppi == true {
    $classvars=get_class_args()
    puppi::ze { 'nodejs':
      ensure    => $nodejs::manage_file,
      variables => $classvars,
      helper    => $nodejs::puppi_helper,
    }
  }


  ### Service monitoring, if enabled ( monitor => true )
  if $nodejs::bool_monitor == true {
    monitor::port { "nodejs_${nodejs::protocol}_${nodejs::port}":
      protocol => $nodejs::protocol,
      port     => $nodejs::port,
      target   => $nodejs::monitor_target,
      tool     => $nodejs::monitor_tool,
      enable   => $nodejs::manage_monitor,
    }
    monitor::process { 'nodejs_process':
      process  => $nodejs::process,
      service  => $nodejs::service,
      pidfile  => $nodejs::pid_file,
      user     => $nodejs::process_user,
      argument => $nodejs::process_args,
      tool     => $nodejs::monitor_tool,
      enable   => $nodejs::manage_monitor,
    }
  }
  ...
  if $nodejs::bool_firewall == true {
    firewall { "nodejs_${nodejs::protocol}_${nodejs::port}":
      source      => $nodejs::firewall_src,
      destination => $nodejs::firewall_dst,
      protocol    => $nodejs::protocol,
      port        => $nodejs::port,
      action      => 'allow',
      direction   => 'input',
      tool        => $nodejs::firewall_tool,
      enable      => $nodejs::manage_firewall,
    }
  }


  ### Debugging, if enabled ( debug => true )
  if $nodejs::bool_debug == true {
    file { 'debug_nodejs':
      ensure  => $nodejs::manage_file,
      path    => "${settings::vardir}/debug-nodejs",
      mode    => '0640',
      owner   => 'root',
      group   => 'root',
